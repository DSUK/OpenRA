...
if [ ! -f StyleCopPlus.dll ]; then
	...
	mv ./StyleCopPlus.MSBuild.4.7.49.5/tools/StyleCopPlus.dll .
...
if [ ! -f StyleCop.dll ]; then
	...
	mv ./StyleCop.MSBuild.4.7.49.0/tools/StyleCop*.dll .
...
if [ ! -f ICSharpCode.SharpZipLib.dll ]; then
	...
	mv ./SharpZipLib.0.86.0/lib/20/ICSharpCode.SharpZipLib.dll .
...
if [ ! -f MaxMind.GeoIP2.dll ]; then
	...
	mv ./MaxMind.Db.1.0.0.0/lib/net40/MaxMind.Db.* .
	rm -rf MaxMind.Db.1.0.0.0
	mv ./MaxMind.GeoIP2.2.1.0.0/lib/net40/MaxMind.GeoIP2* .
	rm -rf MaxMind.GeoIP2.2.1.0.0
	mv ./Newtonsoft.Json.6.0.5/lib/net40/Newtonsoft.Json* .
	rm -rf Newtonsoft.Json.6.0.5
	mv ./RestSharp.105.0.0/lib/net4-client/RestSharp* .
...
if [ ! -f SharpFont.dll ]; then
	...
	mv ./SharpFont.2.5.0.1/lib/net20/SharpFont* .
	mv ./SharpFont.2.5.0.1/Content/SharpFont.dll.config .
...
if [ ! -f nunit.framework.dll ]; then
	...
	mv ./NUnit.2.6.4/lib/nunit.framework* .
	rm -rf NUnit.2.6.4
fi
...
if [ ! -f Mono.Nat.dll ]; then
	...
	mv ./Mono.Nat.1.2.21.0/lib/net40/Mono.Nat.dll .
...
if [ ! -f Moq.dll ]; then
	...
	mv ./Moq.4.2.1502.0911/lib/net40/Moq.dll .
...
if [ ! -f FuzzyLogicLibrary.dll ]; then
	...
	mv ./FuzzyLogicLibrary.1.2.0/bin/Release/FuzzyLogicLibrary.dll .
